...
export default function BarCompare({ label1, val1, label2, val2, unit, color1, color2 }) {
  ...
  const bar = (val, color) => {
    ...
    const inside = pct >= 30;
    return (
      <div style={{ flex: 1, minWidth: 0, minHeight: 28, background: theme.color.bg, borderRadius: 6, overflow: "hidden", display: "flex", alignItems: "center" }}>
        <div style={{ width: `${pct}%`, alignSelf: "stretch", minHeight: 28, background: color, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "flex-end", paddingRight: inside ? 8 : 0, boxSizing: "border-box", fontSize: 12, fontWeight: 600, color: theme.color.white, transition: "width 0.6s ease" }}>
          {inside && `${val.toLocaleString("de-DE")} ${unit}`}
        </div>
        {!inside && (
          <span style={{ paddingLeft: 8, fontSize: 12, fontWeight: 600, color: theme.color.textPrimary, lineHeight: 1.25, padding: "4px 8px 4px 0" }}>
