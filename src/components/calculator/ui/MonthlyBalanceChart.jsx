...
const SERIES = [
  { key: "eigenverbrauch", label: "Eigenverbrauch", color: theme.color.accent },
  { key: "einspeisung", label: "Einspeisung", color: theme.color.sky },
  { key: "netzbezug", label: "Netzbezug", color: theme.color.textMuted },
];
...
export default function MonthlyBalanceChart({ balance }) {
  ...
  return (
    <div style={{ background: theme.color.white, borderRadius: theme.radius.lg, border: `1px solid ${theme.color.border}`, padding: "22px 20px 18px", marginBottom: 16 }}>
      <style>{`
        .mbc-m-kurz { display: none; }
        @media (max-width: 420px) { .mbc-m-lang { display: none; } .mbc-m-kurz { display: inline; } }
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ position: "absolute", bottom: PLOT_H - zeroY + 1, left: "18%", right: "18%", display: "flex", flexDirection: "column-reverse", gap: 2 }}>
                      {eigH > 0 && <div style={{ background: theme.color.accent, height: eigH, borderRadius: einH > 0.5 ? "0" : "4px 4px 0 0" }} />}
                      {einH > 0.5 && <div style={{ background: theme.color.sky, height: einH, borderRadius: "4px 4px 0 0" }} />}
                    </div>
                    {/* Unten: Netzbezug */}
                    {netzH > 0.5 && (
                      <div style={{ position: "absolute", top: zeroY + 1, left: "18%", right: "18%", height: netzH, background: theme.color.textMuted, borderRadius: "0 0 4px 4px" }} />
                    )}
                  </div>
                );
              })}
          ...
          <div style={{ display: "flex", marginTop: 8 }} aria-hidden="true">
            {balance.map((m, i) => (
              <div key={m.month} style={{ flex: 1, minWidth: 0, fontSize: 11, textAlign: "center", whiteSpace: "nowrap", overflow: "visible", color: active === i ? theme.color.textPrimary : theme.color.textMuted, fontWeight: active === i ? 600 : 400 }}>
                <span className="mbc-m-lang">{MONTH_LABELS[m.month - 1]}</span><span className="mbc-m-kurz">{MONTH_LABELS[m.month - 1][0]}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8, marginTop: 18, paddingTop: 14, borderTop: `1px solid ${theme.color.border}` }}>
        {SERIES.map((s) => (
          <div key={s.key}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: theme.color.textSecondary, minWidth: 0 }}>
              <span style={{ width: 12, height: 2, background: s.color, display: "inline-block", flexShrink: 0 }} /><span style={{ minWidth: 0, overflowWrap: "anywhere", hyphens: "auto" }} lang="de">{s.label}</span>
            </div>
