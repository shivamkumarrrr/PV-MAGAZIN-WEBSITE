...
export default function StepSpeicher({ speicherKwh, setSpeicherKwh, speicherWahl, setSpeicherWahl, kwp, gesamtVerbrauch, vorschlagKwh, tageszeit }) {
  ...
  const Karte = ({ wahl, titel, sub, pct, Icon }) => {
    ...
    return (
      ...
      >
        <span style={{ width: 48, height: 48, borderRadius: theme.radius.md, display: "flex", alignItems: "center", justifyContent: "center", background: active ? theme.color.accent : theme.color.bg, color: active ? theme.color.white : theme.color.textSecondary }}>
          <Icon size={26} />
        </span>
  ...
  return (
    <>
      <div style={{ fontSize: 16, color: theme.color.textPrimary, fontWeight: 600, marginBottom: 6, lineHeight: 1.35 }}>Möchten Sie einen Batteriespeicher?</div>
